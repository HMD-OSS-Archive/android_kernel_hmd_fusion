...
{
	smartpin_role = role;
	pr_err("%s: smartpin_role = %s;\n", __func__, usb_role_string(smartpin_role));

	return 0;
}
EXPORT_SYMBOL(smartpin_set_role);

int smartpin_get_role(void)
{
	if(smartpin_role == USB_ROLE_NONE)
		return 0;
	else if(smartpin_role == USB_ROLE_HOST)
		return 1;
	else if(smartpin_role == USB_ROLE_DEVICE)
		return 2;

	return -1;
}
EXPORT_SYMBOL(smartpin_get_role);
